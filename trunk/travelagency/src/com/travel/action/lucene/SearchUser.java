 ...
 * @version 1.0
 **/
public class SearchUser extends BaseAction {
	private Logger logger=LoggerFactory.getLogger(this.getClass());
	private String IDNEX_PATH = ConfigUtil.get("indexdir")+"/adminuser";
	private BooleanClause.Occur[] flags = {BooleanClause.Occur.SHOULD,BooleanClause.Occur.SHOULD,BooleanClause.Occur.SHOULD};
	...
	public String execute() {
		...
		try {
			results =BaseSearch.search(IDNEX_PATH,new String[] {searchword,searchword,searchword}, new String[] {"id","user_name","description"}, new String[] {"id","user_name","description"},flags,pageNo,true);
			if (results!=null) {
				pageNo=results.getPageNo();
				logger.debug("结果数量为"+results.getResults().size());
			}
