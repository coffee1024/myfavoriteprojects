 ...
 * SimpleHibernateDao.java 762 2009-12-27 14:46:36Z calvinxiu $
 */
...
public class SimpleHibernateDao<T, PK extends Serializable> {
	...
	/**
	 * 用于用于省略Dao层, 在Service层直接使用通用SimpleHibernateDao的构造函数. 在构造函数中定义对象类型Class. eg. SimpleHibernateDao<User, Long>
	 ...
	 * 仅保存新增对象(生成全文索引).
	 */
	public void insertFullTesxt(final T entity) {
		Assert.notNull(entity, "entity不能为空");
		getFullTextSession().save(entity);
		logger.debug("insert entity: {}", entity);
	}
	/**
	 * 保存新增或修改的对象.
	 ...
	 *            对象必须是session中的对象或含id属性的transient对象.
	 */
	public void deleteFullText(final T entity) {
		Assert.notNull(entity, "entity不能为空");
...
//		return (T) getSession().load(entityClass, id);
	 ...
	 * 按id获取对象.（全文）
	 */
	public T getFullTest(final PK id) {
		Assert.notNull(id, "id不能为空");
//		return (T) getSession().load(entityClass, id);
		return (T) getFullTextSession().get(entityClass, id);
	}
