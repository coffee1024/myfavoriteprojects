...
public class StringUtils {
	protected final Logger logger = LoggerFactory.getLogger(StringUtils.class);
	...
	public static String stringFilter(String str) throws PatternSyntaxException {
		//weixing 该正则中过滤掉了文件名为201009121284225070179_p.jpg中的 . 符号，导致无法检索无效
		//String regEx = "[`~!@#$%^&*()+=|{}':;',\\[\\].<>/?~！@#￥%……&*（）——+|{}【】‘；：”“’。，、？]";
		//^ $ . * + ? = ! : | / ( ) [ ] { } 
//		String regEx = "[`~!@#$%^&*()+=|{}':;',\\[\\]<>/?~！@#￥%……&*（）——+|{}【】‘；：”“’。，、？]";
		String regEx = "[`~!@#$%^&*()+=|{}':;',\\[\\]<>/?~！@#￥%……&*（）——+|{}【】‘；：”“’。，、？]";
	 ...
	 * @param s 要查找的字符或字符串
	 */
	public static boolean isStringInArray(String[] strs, String s) {
	...
	public static void main(String[] args) {
		...
		String[] strs = new String[]{"中午a%\\", "trs","ab"};
		String s = "中午a%\\a";
		System.out.println("判断字符串是否存在在数组中1: "+ isStringInArray(strs, s));
		System.out.println("判断字符串是否存在在数组中2: "+ 
				org.apache.commons.lang.ArrayUtils.indexOf(strs, s)); 
		System.out.println("判断字符串是否存在在数组中3: "+ 
				org.apache.commons.lang.ArrayUtils.contains(strs, s));
	}
	/**
	 * 判断是否为中文字符
	 * @param oneChar
	 ...
	 * 截取字符串，只适用于收藏夹名的显示
	 * @param str 初始字符串
	 * @param maxLength 最大长度，一个中文算两个长度
	 * @param end 添加的字符串尾巴
	 * @return 
	 */
	public static String cutString(String str,int maxLength,String end){
		char[] chars=str.toCharArray();
		int temp=0;
		//计算字符串占位长度，一个中文=两个英文=2
		...
		//当字符长度小雨等于需要截断的长度时，返回原字符串，不加截断标记
		if (temp<=maxLength) {
			return str;
		}
		int length=0;
